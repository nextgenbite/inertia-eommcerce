...
function uploadAttributeImage(event, index) {
  const file = event.files[0];
  const reader = new FileReader();
  reader.onload = () => {
    form.attribute_images[index].image_path = file; // Actual file to send
    form.attribute_images[index].preview = reader.result; // For preview
  };
...
watchEffect(() => {
  if (props.show) {
    ...
    form.sub_category_id = props.data?.sub_category_id ?? "";
    form.sub_sub_category_id = props.data?.sub_sub_category_id ?? "";

    const selectedCategory = categoryOptions.find(
      (cat) => cat.id === form.category_id
    );
    subcategoryOptions.value = selectedCategory?.sub_categories || [];

    const selectedSub = subcategoryOptions.value.find(
      (sub) => sub.id === form.sub_category_id
    );
    subSubcategoryOptions.value = selectedSub?.sub_sub_categories || [];

    form.brand_id = props.data?.brand_id;
    form.unit_id = props.data?.unit_id;
    form.sku = props.data?.sku;
    form.barcode = props.data?.barcode;
    form.weight = props.data?.weight;
    form.length = props.data?.length;
    form.width = props.data?.width;
    form.height = props.data?.height;

    imagePreview.value = props.data?.thumbnail
      ? `/${props.data.thumbnail}`
      : "/no-image.png";
    form.thumbnail = null;

    // Convert image strings to File[] if needed
    form.images = [];

    form.buying_date = props.data?.buying_date;
    form.expire_date = props.data?.expire_date;
    form.buying_price = props.data?.buying_price;
    form.discount_price = props.data?.discount_price;
    form.price = props.data?.price;
    form.has_variants = props.data?.has_variants;
    form.attribute_images = props.data?.attribute_images || [];
    form.variants = props.data?.variants || [];
    form.min_qty = props.data?.min_qty;
    form.max_qty = props.data?.max_qty;
              ...
              >
                ...
                <div class="md:col-span-2 flex flex-col gap-4">
                  ...
                  <MultipleImageUpload
                    ...
                    class="w-full"
                     :existingImages="parseImages(data.images)" @update:existingImages="form.images = $event"
                  />
                  <small
